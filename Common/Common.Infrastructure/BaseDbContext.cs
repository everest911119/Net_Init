...
namespace Common.Infrastructure
{
    public abstract class BaseDbContext : DbContext
    {
        ...
        public override async Task<int> SaveChangesAsync(bool acceptAllChangesOnSuccess, CancellationToken cancellationToken = default)
        {
            ...
            if (mediator != null)
            {
               await mediator.DispatchDomainEventAysnc(this);
            }
            //在提交到数据库之前，记录那些被“软删除”实体对象
            var softDeletedEntities = this.ChangeTracker.Entries<ISoftDelete>().
                Where(e => e.State == EntityState.Modified && e.Entity.IsDelete)
            ...
            //因为FindAsync如果能从本地Cache找到，
            //就不会去数据库上查询，而从本地Cache找的过程中不会管QueryFilter
            //就会造成已经软删除的数据仍然能够通过FindAsync查到的问题，
            //因此这里把对应跟踪对象的state改为Detached，就会从缓存中删除了
            return res;
        }
    }
}
